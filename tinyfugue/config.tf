...
; Pages
/def -p2 -aCcyan -P1B -t"^([^\s]+) pages,?" hl_page1
...
; Whispers
/def -p2 -aCblue -P1B     -t"^([^\s]+) whispers,?" hl_whisper1
/def -p3 -aCblue -mregexp -t'^You whisper,?' hl_whisper2

; Looks, smells, etc
/def -p5 -aCmagenta -P1B -t"([^\s]+) (?:just )?looked at you" hl_look1
/def -p5 -aCmagenta -P1B -t"([^\s]+) (?:just )?smell(?:s|ed) you" hl_smell1
